...
exports.language = P.createLanguage({
    ...
    centerHtmlTag: r => {
        const open = P.str('<center>');
        const close = P.str('</center>');
        return P.seq(newLine.option(), P.lineBegin, open, newLine.option(), P.seq(P.notMatch(P.seq(newLine.option(), close)), nest(r.inlineHtml)).select(1).many(1), newLine.option(), close, P.lineEnd, newLine.option()).map(result => {
            return M.CENTER((0, util_1.mergeText)(result[4]));
        });
    },
    big: r => {
